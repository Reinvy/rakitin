...
function detectProject(root = process.cwd()) {
  ...
  // --- Configuration ----------------------------------------------------
  const configCandidates = [".rakitinrc.json", ".rakitinrc", "rakitin.config.json"];
  const configFile = configCandidates.find((c) => fs.existsSync(path.join(root, c)));
  let parsedConfig = null;
  if (configFile) {
    try {
      parsedConfig = JSON.parse(fs.readFileSync(path.join(root, configFile), "utf8"));
    } catch {
      parsedConfig = null;
    }
  }
  ...
  return {
    ...
    config: {
      ...
      preset: parsedConfig ? parsedConfig.preset : null,
      orm: parsedConfig ? parsedConfig.orm || parsedConfig.defaultORM || null : null,
      defaultArchitecture: parsedConfig
        ? parsedConfig.defaultArchitecture || parsedConfig.arch || null
        : null,
    },
  };
}
