...
async function integrateCommand(options = {}) {
  ...
  for (const m of mwList) {
    ...
    const filePath = path.join(p.sharedPath, "middlewares", `${fileName}.middleware.js`);
  ...
  return {
    ...
    nextSteps: ["Pasang router ke express app: app.use('/api', require('./app/routes'))"],
  };
}

function resolveMiddlewareList(mw) {
  if (!mw) return [];
  return Array.isArray(mw)
    ? mw
    : String(mw)
        .split(",")
        .map((s) => s.trim())
