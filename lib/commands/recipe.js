...
async function authRecipe(options = {}) {
  ...
  // 2. User module (configured ORM, defaults to Prisma)
  const archFn = options.arch === "simple" ? simpleArch : modularArch;
  await archFn("user", ormName);
  ...
  // Generate ORM artifacts if ORM is active
  if (ormName !== "None") {
    ...
    if (ormFn) {
      await ormFn("user", options.arch === "simple" ? "Simple" : "Modular");
    }
  }
  ...
  const depsToInstall = ["middleware:auth", "validation:joi"];
  if (ormName !== "None") {
