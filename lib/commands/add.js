...
async function addCommand(thing, name, context) {
  switch (thing) {
    ...
    case "endpoint":
      return generateEndpointCmd(name, context?.resource, context?.fields, context?.pagination, context?.filtering);
    case "validation":
      return generateValidationCmd(name, context?.validatorName, context?.fields);
    case "docs":
      return generateDocumentationCmd(name, context?.title, context?.version, context?.auth);
    default:
...
async function addModule(name, context) {
  ...
  const needPrompts = !moduleName || !architecture || orm === undefined || orm === null;
