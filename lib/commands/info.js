...
function doctorCommand() {
  ...
  checks.push({
    ...
    detail: s.hasAppBase ? "app/ ditemukan" : "app/ belum ada (akan dibuat saat generate)",
  });

  const moduleStatus =
    s.mixedArchitectures ? "warn" : modulesCount(s) > 0 ? "ok" : "info";
  checks.push({
    name: "Modul",
    status: moduleStatus,
    detail:
      modulesCount(s) === 0
        ...
        : `${s.modularCount} modular · ${s.simpleCount} simple${
            s.mixedArchitectures ? " (arsitektur campuran - auto-router tetap mendukung)" : ""
          }`,
  });
...
const CATALOG = [
  { command: "add module <name>", tiers: ["basic", "intermediate", "advanced"], desc: "Controller/service/router (+ORM wiring di tier atas)" },
  { command: "add middleware <kind>", tiers: ["basic"], kind: ["custom", "auth", "logger", "error", "request-time"], desc: "Middleware Express siap pakai" },
  { command: "add util <kind>", tiers: ["basic"], kind: ["custom", "date", "string", "number", "array", "object", "file", "crypto", "uuid", "env", "url", "color", "math", "validation", "regex", "time"], desc: "Utility fungsi umum" },
  { command: "add config <kind>", tiers: ["basic"], kind: ["app", "database", "jwt", "cors", "logger", "mailer", "cloud", "payment", "redis", "socket", "env", "custom"], desc: "Config file berbasis env" },
  { command: "add endpoint <resource>", tiers: ["intermediate"], kind: ["pagination", "filtering", "joi"], desc: "CRUD endpoint untuk modul existing" },
  { command: "add validation [name]", tiers: ["intermediate"], kind: ["module", "new", "common"], desc: "Skema validasi Joi" },
  { command: "add docs <kind>", tiers: ["advanced"], kind: ["openapi-json", "openapi-yaml", "swagger-ui", "complete"], desc: "OpenAPI/Swagger scaffolding" },
  { command: "recipe auth", tiers: ["advanced"], desc: "JWT auth lengkap (middleware + user module + joi)" },
  { command: "recipe swagger", tiers: ["advanced"], desc: "OpenAPI 3 + swagger-ui terhubung modul" },
  { command: "recipe test", tiers: ["advanced"], desc: "Scaffold jest + supertest per modul" },
  { command: "recipe docker", tiers: ["advanced"], desc: "Dockerfile multi-stage + compose" },
  { command: "integrate", tiers: ["basic"], desc: "Sambungkan router utama (marker-based, idempotent)" },
];
