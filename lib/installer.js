/**
 * Package Installer - Async package installation with retry logic
 * Supports: npm, pnpm, yarn with exponential backoff
 */
...
async function executeWithRetry(command, options = {}) {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      const result = await execCommand(command, { stdio });

      logger.debug(`Command succeeded on attempt ${attempt + 1}`);
      return { success: true, ...result };
    } catch (error) {
      lastError = error;
      logger.warn(`Command failed (attempt ${attempt + 1}/${maxRetries + 1}): ${error.message}`);
...
async function installIfNeeded(packageNames = [], options = {}) {
  ...
  if (!silent) {
    logger.info(`Installing ${packagesToInstall.length} packages: ${packagesToInstall.join(', ')}`);
  ...
  try {
    ...
    if (execResult.success) {
      ...
      const errorMessage = createErrorMessage("system", `installing packages: ${execResult.stderr}`);
      if (!silent) {
    ...
    const errorMessage = createErrorMessage("system", `installing packages: ${error.message}`);
    if (!silent) {
      logger.error(errorMessage);
    }
  }
...
async function installOrmPackages(orm, options = {}) {
  ...
  return installIfNeeded(ormPackages[orm], options);
}
