...
module.exports = {
  ...
  // Tanyakan apakah ingin membuat file .env.example
  ...
  if (createEnvExample) {
    const envExampleContent = getEnvExampleContent(configType);
    ...
    if (!fs.existsSync(envExamplePath)) {
      ...
      // Jika file sudah ada, tambahkan konten baru
      const existingContent = fs.readFileSync(envExamplePath, "utf8");
      if (!existingContent.includes(`# ${configType.toUpperCase()} CONFIG`)) {
