...
function sharedPath() { return getPaths().sharedPath; }
const { toPascalCase, toKebabCase } = require("../../../utils");

async function generateValidation() {
  const { validatorType } = await inquirer.default.prompt([
    {
      type: "list",
      name: "validatorType",
  ...
  if (validatorType === "from-module") {
    await generateFromModule();
  } else if (validatorType === "new") {
    await generateNewValidator();
  } else {
    await generateCommonValidators();
  }
}

async function generateFromModule() {
  ...
  const { fieldsInput } = await inquirer.default.prompt([
    {
      type: "input",
      name: "fieldsInput",
      message: "Field schema (format: name:type:required, contoh: email:string:true,password:string:true):",
...
async function generateNewValidator() {
  const { validatorName } = await inquirer.default.prompt([
    {
      type: "input",
      name: "validatorName",
...
${fields.filter(f => f.required).map(f => `  ${f.name}: Joi.${mapToJoiType(f.type, f.name)}.required()`).join(",\n")}
});
...
function mapToJoiType(type, fieldName) {
  ...
  if (fieldName.toLowerCase().includes("url") || fieldName.toLowerCase().includes("link")) return "string().uri()";
  if (fieldName.toLowerCase().includes("age") || fieldName.toLowerCase().includes("count")) return "number().integer()";
