...
const { ensureDir, toPascalCase, toKebabCase } = require("../../../utils");
...
async function generateValidation(typeArg, nameArg, fieldsArg) {
  ...
  if (typeArg && modules.includes(typeArg)) {
    validatorType = "from-module";
    nameArg = typeArg;
  }
  ...
  if (validatorType === "from-module") {
    result = await generateFromModule(nameArg || (modules.length > 0 ? modules[0] : null), fieldsArg);
  } else if (validatorType === "new") {
    result = await generateNewValidator(nameArg, fieldsArg);
  } else {
    result = await generateCommonValidators();
  }

  return {
    created: true,
    ...(result || {}),
    nextSteps: ["Gunakan schema validasi di middleware route Anda."],
  };
}
