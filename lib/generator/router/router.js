...
`;
  ...
  try {
    ...
    const { content: finalContent, action } = buildRoutesContent(
      existing,
      routeLines
    );
    ...
    console.log(`✅ Router utama ${action === "create" ? "dibuat" : "diperbarui"} di ${appRouterPath}`);
...
async function createAutoRouter(architecture = 'modular', middlewares = []) {
  try {
    ...
    const existing = fs.existsSync(appRouterPath)
      ...
      : null;
    const routeLines = extractRouteLines(content);
    const { content: finalContent } = buildRoutesContent(
      existing,
      routeLines
    );
