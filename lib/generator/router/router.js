...
/** Extract the inner lines of the generated static router (both branches). */
function extractRouteLines(content) {
  // Drop header and the final export; keep imports, middleware, wiring.
  const withoutHeader = content
    .replace(/^const express = require\('express'\);\s*/, "")
    .replace(/^const router = express\.Router\(\);\s*/, "");
...
`;
      ...
      modules.forEach((m) => {
        ...
        if (!routerValidation.isValid) {
          ErrorHandler.handleFileNotFoundError(
            routerValidation.path,
...
`;
      ...
      modules.forEach((m) => {
        ...
        if (!controllerValidation.isValid) {
          ErrorHandler.handleFileNotFoundError(
            controllerValidation.path,
            "Simple Controller Import"
          );
          return;
        }
...
`;
    ...
    // Safety layer: never blind-overwrite an existing main router.
    // Existing file (with or without markers) -> inject via markers with
    // a .bak backup; missing file -> fresh create.
    try {
      const existing = fs.existsSync(appRouterPath)
        ? fs.readFileSync(appRouterPath, "utf8")
        : null;
      const routeLines = extractRouteLines(content);
      const { content: finalContent, action } = buildRoutesContent(existing, routeLines);

      if (existing == null) {
        writeFileIfNotExistsSafe(appRouterPath, finalContent);
      } else {
        overwriteWithBackup(appRouterPath, finalContent);
      }
      console.log(
        `✅ Router utama ${action === "create" ? "dibuat" : "diperbarui"} di ${appRouterPath}`
      );
...
`;
    ...
    console.log(
      "❌ Integrasi router gagal. Silakan periksa error log untuk detail lebih lanjut."
    );
  }
}
...
async function createAutoRouter(architecture = "modular", middlewares = []) {
  try {
    ...
    // Marker-aware: an existing router (with our markers) gets its managed
    // block regenerated; a marker-less router gets the block appended with
    // backup; missing file is created fresh.
    const existing = fs.existsSync(appRouterPath)
      ? fs.readFileSync(appRouterPath, "utf8")
      : null;
    const routeLines = extractRouteLines(content);
    const { content: finalContent } = buildRoutesContent(existing, routeLines);
    ...
    console.log(
      "✅ Router akan secara otomatis mendeteksi dan mengimpor semua modul yang tersedia"
    );
    ...
    console.log(
      "❌ Gagal membuat auto router. Silakan periksa error log untuk detail lebih lanjut."
    );
    return false;
  }
}
...
async function integrateAutoRouter(options = {}) {
  try {
    const { autoDetect = true, architecture = "modular", middlewares = [] } = options;
    ...
    console.log(
      "❌ Integrasi auto router gagal. Silakan periksa error log untuk detail lebih lanjut."
    );
    return false;
  }
}
