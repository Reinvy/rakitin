...
const { ensureDir, toKebabCase } = require("../../utils");
...
async function generateUtil(utilTypeArg, customNameArg) {
  let utilType = utilTypeArg;
  if (!utilType) {
  ...
  if (utilType.toLowerCase() === "custom") {
    let nameVal = customNameArg;
    if (!nameVal) {
    ...
    name = toKebabCase(nameVal || "custom");
    content = `// Util: ${name}
...
module.exports = { exampleFunction };`;
    ...
    const predefined = getUtilContent(utilType.toLowerCase());
    name = `${toKebabCase(utilType)}.util`;
    content = predefined ? predefined.content : `// Util: ${name}\n\nmodule.exports = {};`;
  }

  const filePath = path.join(utilDir, `${name}.js`);
  const { written } = writeFileIfNotExistsSafe(filePath, content);
  const createdFiles = written ? [filePath] : [];

  if (written) {
    console.log(`✅ Util '${name}' berhasil dibuat di 'app/shared/utils/${name}.js'!`);
  } else {
  ...
  return {
    ...
    createdFiles,
    nextSteps: [
      `Import util di service/controller: require('../shared/utils/${name}')`,
    ],
  };
}
