...
class FileValidator {
  ...
  static validateModuleDirectory(moduleName, basePath, architecture = "modular") {
    try {
      ...
      // Untuk arsitektur modular, hanya 'routes' yang benar-benar dipakai
      // oleh integrasi router. Memaksa controllers/services/models membuat
      // modul minimal-yet-valid (mis. routes-only) gagal diverifikasi.
      if (architecture === "modular") {
        const dirPath = path.join(modulePath, "routes");
  ...
  static validateJavaScriptFile(filePath) {
    try {
      ...
      // Compile-only syntax validation via vm.Script.
      // NEVER require() the target file here: executing arbitrary project
      // code as a "syntax check" is a security hazard (side effects, env
      // access, DB connections). vm.Script catches SyntaxErrors only.
      try {
        ...
        return {
          isValid: false,
          error: `Syntax error dalam file: ${syntaxError.message}`,
        };
      }
  ...
  static validateRouterIntegration(modules, basePath, architecture = "modular") {
    ...
    for (const moduleName of modules) {
      ...
      const moduleValidation = this.validateModuleDirectory(
        moduleName,
        basePath,
        architecture
      );
      ...
      if (architecture === "modular") {
        ...
        const controllerValidation = this.validateSimpleControllerFile(
          moduleName,
          basePath
        );
