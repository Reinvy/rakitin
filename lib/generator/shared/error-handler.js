...
class ErrorHandler {
   ...
   * @param {string} context - Konteks error
   */
  static handleModuleValidationError(
    moduleName,
    validationError,
  ...
  static getSuggestion(errorType) {
    const suggestions = {
      [this.ERROR_TYPES.FILE_NOT_FOUND]:
        "Periksa kembali path file dan pastikan file tersebut ada.",
      [this.ERROR_TYPES.INVALID_PATH]: "Pastikan format path yang diberikan valid.",
      [this.ERROR_TYPES.MODULE_VALIDATION]:
        "Periksa kembali nama modul dan parameter yang diberikan.",
      [this.ERROR_TYPES.ROUTER_INTEGRATION]:
        "Pastikan semua file yang diperlukan sudah ada dan valid.",
      [this.ERROR_TYPES.FILE_CREATION]:
        "Periksa izin akses direktori dan pastikan path valid.",
      [this.ERROR_TYPES.DIRECTORY_CREATION]:
        "Periksa izin akses dan pastikan parent directory ada.",
      [this.ERROR_TYPES.IMPORT_ERROR]:
        "Periksa sintaks file dan dependensi yang diperlukan.",
      [this.ERROR_TYPES.VALIDATION_ERROR]: "Periksa kembali input yang diberikan.",
    };

    return (
      suggestions[errorType] || "Periksa kembali konfigurasi dan input yang diberikan."
    );
  }
}
