...
async function remove(req) {
  ...
  return await ${camelName}Model.findByIdAndDelete(id);
      ...
      case "None":
        // No-ORM mode handled by the dedicated EJS template (Phase A of
        // the inline->EJS migration). Kept out of generateServiceOperations
        // so the switch below stays the single dispatch point.
        throw new Error("None should be rendered via renderModuleTemplate");
...
module.exports = { getAll, getById, create, update, remove };`;
    ...
    case "None":
      // Phase A: rendered from lib/templates/module/service-none.ejs
      return renderModuleTemplate("service-none.ejs", {
        moduleName,
        storeName: `${camelName.toUpperCase()}_STORE`,
      });
