...
async function generateModule() {
  try {
    let defaultOrm = "Prisma";
    let defaultArch = "Modular";
    try {
      const config = new Config().load(process.cwd());
      const configuredOrm = config.get("orm") || config.get("defaultORM");
      if (configuredOrm) {
        defaultOrm =
          configuredOrm.toLowerCase() === "typeorm"
            ? "TypeORM"
            : configuredOrm.charAt(0).toUpperCase() + configuredOrm.slice(1).toLowerCase();
      }
      const configuredArch = config.get("defaultArchitecture");
      if (configuredArch) {
        defaultArch = configuredArch.toLowerCase() === "simple" ? "Simple" : "Modular";
      }
    } catch {
    ...
    const {
      ...
      },
      {
        ...
        choices: ["Yes", "No"],
        default: defaultOrm === "None" ? "No" : "Yes",
      },
      {
        type: "select",
        name: "orm",
        ...
        default: defaultOrm === "None" ? "Prisma" : defaultOrm,
