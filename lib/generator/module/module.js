...
async function generateModule() {
  try {
    const { moduleName, architecture, useORM, orm, autoIntegrateRouter, routerLocation, routerArchitecture } =
      await inquirer.default.prompt([
        ...
        },
        {
          type: "list",
          name: "routerLocation",
          message: "Di mana router utama berada?",
          choices: [
            { name: "Di folder app/routes", value: "app" },
            { name: "Di folder root", value: "root" },
            { name: "Buat baru di folder app/routes", value: "app_new" },
            { name: "Buat baru di folder root", value: "root_new" },
          ],
          default: "app",
    ...
    if (autoIntegrateRouter) {
      await handleAutoRouterIntegration(moduleName, architecture, routerLocation, routerArchitecture);
    }

    console.log(
      `\n✅ Modul "${moduleName}" berhasil dibuat dengan arsitektur ${architecture}${useORM === "Yes" ? ` dan ORM ${orm}` : ''}${autoIntegrateRouter ? ' dan sudah diintegrasikan ke router utama' : ''}\n`
    );
  } catch (error) {
  ...
  if (useORM === "Yes") {
    ...
    if (ormFunctions[orm]) {
      ...
      if (installResult.success) {
        console.log(`🎉 Package ${orm} berhasil diinstall dan akan diintegrasikan ke dalam modul ini.`);
      } else {
        console.error(`❌ Gagal menginstall beberapa package untuk ${orm}: ${installResult.failed.join(", ")}`);
      }
    } else {
 ...
 */
async function handleAutoRouterIntegration(moduleName, architecture, routerLocation, routerArchitecture) {
  try {
    const fs = require("fs");
    ...
    if (routerLocation === "app") {
      ...
      if (!fs.existsSync(appRouterPath)) {
        console.log("⚠️  Router utama tidak ditemukan di lokasi yang dipilih.");
        return;
      }
    } else if (routerLocation === "root") {
    ...
    } else if (routerLocation === "app_new") {
      appRouterPath = path.join(basePath, "routes", "index.js");
      isNewRouter = true;
    ...
    } else if (routerLocation === "root_new") {
      appRouterPath = path.join(process.cwd(), "routes", "index.js");
      isNewRouter = true;
    ...
    const validation = FileValidator.validateRouterIntegration(
      [moduleName],
      routerLocation.includes("app") ? basePath : process.cwd(),
      routerArchitecture
    );
...
`;
        ...
        ErrorHandler.handleFileCreationError(appRouterPath, error, "Main Router Update");
        console.log(`❌ Gagal memperbarui file router utama di ${appRouterPath}`);
        return;
      }
    }
...
`;
    ...
    console.log("❌ Integrasi otomatis router gagal. Silakan periksa error log untuk detail lebih lanjut.");
  }
}
