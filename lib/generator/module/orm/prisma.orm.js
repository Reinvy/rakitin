...
function updatePackageJsonWithPrismaSchema() {
  try {
    ...
    console.log("📝 Properti 'prisma.schema' berhasil ditambahkan ke package.json.");
  } catch (error) {
...
async function checkAndInitPrisma() {
  try {
    ...
    console.log("⚠️ Prisma belum terinisialisasi. Menjalankan 'npx prisma init'...");

    try {
      child_process.execSync("npm install prisma@^7 @prisma/client@^7", { stdio: "inherit" });
      child_process.execSync("npx prisma init", { stdio: "inherit" });
      console.log("🎉 Prisma berhasil diinisialisasi!");
      updatePackageJsonWithPrismaSchema();
    } catch (error) {
      throw new Error(`Gagal inisialisasi Prisma: ${error.message}`, { cause: error });
    }
  } catch (error) {
