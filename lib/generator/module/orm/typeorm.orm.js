...
`;
    ...
    writeFileIfNotExists(entityPath, boilerplate.trimStart());
    console.log(
...
async function checkAndInstallTypeORM() {
  try {
    ...
    try {
      ...
      throw new Error(`Gagal menginstal TypeORM: ${error.message}`, { cause: error });
    }
  } catch (error) {
...
`;
    ...
    writeFileIfNotExists(dataSourcePath, dataSourceBoilerplate.trimStart());
    console.log(
