...
async function typeormORM(moduleName, architecture) {
  try {
    ...
    if (architecture === "Modular") {
      entityPath = path.join(modulesPath, kebabName, "entities", `${kebabName}.entity.js`);
...
async function ensureDataSourceFile() {
  try {
    const dataSourcePath = path.join(process.cwd(), "app", "shared", "config", "data-source.js");
...
`;
    ...
    console.log(`✅ File data-source.js berhasil dibuat di '${path.dirname(dataSourcePath)}'.`);
  } catch (error) {
