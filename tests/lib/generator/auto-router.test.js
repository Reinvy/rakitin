...
describe("Auto Router Generator", () => {
  describe("createAutoRouterTemplate", () => {
    ...
    test("middleware requires point at shared/middlewares with valid ids", () => {
      ...
      expect(template).toContain("require('../shared/middlewares/auth.middleware')");
      expect(template).toContain("require('../shared/middlewares/logger.middleware')");
    ...
    test("parses as JavaScript for every middleware combination", () => {
      ...
      expect(
        () =>
          new vm.Script(
            createAutoRouterTemplate("simple", [
              "auth",
              "logger",
              "error",
              "request-time",
